...
#endif
			//if(byte&0x01){////debug
				//printf("K0ON[%02x_%02x]\r\n", 0, 0);
			//}////debug
			//for (int c = 0; c < 8; c++, mask <<= 1)////debug
			for (int c = 0; c < 1; c++, mask <<= 1)
			{
				if ((byte & mask) != 0)
				{
#ifdef DEBUGGER
...
void S9xSetAPUControl (uint8 byte)
{
    ...
    if ((byte & 1) != 0 && !APU.TimerEnabled [0])
    {
		...
		//printf("Timer0 enable[%0xh],", APU.TimerTarget [0]);////debug
    }
    if ((byte & 2) != 0 && !APU.TimerEnabled [1])
    {
		...
		//printf("Timer1 enable[%0xh],", APU.TimerTarget [1]);////debug
    }
    if ((byte & 4) != 0 && !APU.TimerEnabled [2])
    {
		...
		//printf("Timer2 enable[%0xh],", APU.TimerTarget [2]);////debug
    }
